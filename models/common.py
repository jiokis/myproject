...
class CurvatureAwareConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=0, groups=1, r_base=8):
        super().__init__()
        self.base_conv = nn.Conv2d(c1, c2, k, s, p, groups=groups)
        self.r_base = r_base  # 基础秩
        self.curvature_estimator = nn.Sequential(  # 曲率估计器
    ...
    def forward(self, x):
        # 1. 计算局部曲率积分
        K = self.curvature_estimator(x)  # 曲率值
        curvature_integral = torch.mean(torch.abs(K))  # ∫|K(P)|dP

        # 2. 动态调整秩r（曲率波动≤5%）
        if curvature_integral > 0.05:  # 高曲率区域（小目标/边缘）
            r = self.r_base * 2  # 提升秩以保持拓扑连通
        else:
            r = max(4, self.r_base // 2)  # 低曲率区域降秩轻量化

        # 3. 低秩分解卷积（固定基础核+动态调整矩阵）
        weight = self.base_conv.weight
        U, S, Vh = torch.svd(weight.view(weight.shape[0], -1))  # 奇异值分解
        S_r = S[:r] + 1e-8  # 保留前r个奇异值
...
class RiemannSampling(nn.Module):
    ...
    def forward(self, x):
        ...
        windows = windows.view(b, c, 4, h // 2, w // 2)  # (b, c, 4, h', w')

        # 计算中心像素与窗口内其他像素的距离
        center = windows[:, :, 0:1, ...]  # 中心像素
        dist = torch.norm(windows - center, dim=1, keepdim=True)  # 黎曼距离（简化）

        # 采样距离最小的像素（保留局部拓扑）
        min_idx = torch.argmin(dist, dim=2, keepdim=True)  # (b, 1, 1, h', w')
        sampled = torch.gather(windows, dim=2, index=min_idx.expand(-1, c, -1, -1, -1)).squeeze(2)
        return sampled
...
class HomotopyCheck(nn.Module):
    ...
    def forward(self, x_list):
        if not self.training:
            return x_list  # 推理时不验证

        # 计算各层特征流形的欧拉示性数（简化为连通分量计数）
        chis = []
        for x in x_list:
            # 二值化特征（简化拓扑分析）
            x_bin = (torch.sigmoid(x) > 0.5).float()
            # 计算连通分量（近似欧拉示性数）
            components = self.count_components(x_bin)
            chis.append(components)
...
class CurvatureAwareConv(nn.Module):
    ...
    def forward(self, x):
        # 曲率自适应调整（简化版：基于特征方差动态调整卷积核）
        x_var = torch.var(x, dim=(2, 3), keepdim=True)  # 计算特征曲率（方差近似）
        r = int(self.r_base * (1 + torch.sigmoid(self.curvature) * x_var.mean()))  # 动态秩
        r = max(1, min(r, self.base_conv.out_channels))  # 约束范围

        # 低秩近似卷积（替代SVD，提升效率）
        if r < self.base_conv.out_channels:
            weight = self.base_conv.weight[:r]  # 取前r个核
            bias = self.base_conv.bias[:r] if self.base_conv.bias is not None else None
            return F.conv2d(x, weight, bias, self.base_conv.stride, self.base_conv.padding)
        else:
...
class HomotopyCheck(nn.Module):
    ...
    def forward(self, x_list):
        ...
        chis = [self.count_components(x) for x in x_list]
        chi_ref = chis[0]  # 以第一个尺度为参考
        # 软约束：超阈值部分计入损失（替代断言）
        self.homotopy_loss = 0.0
        for chi in chis[1:]:
            diff = torch.abs(chi - chi_ref) / (chi_ref + 1e-6)  # 相对差异
            self.homotopy_loss += F.relu(diff - self.tol)  # 仅惩罚超阈值部分
        return x_list
