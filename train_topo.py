...
def compute_euler_characteristic(feat_map, threshold=0.5):
    ...
    b, c, _h, _w = binary_map.shape
    total = 0
    for i in range(b):
        for j in range(c):
            _labeled, num = label(binary_map[i, j].cpu().numpy())  # 已提前导入label
            total += num  # 简化为连通分量数
...
def train_topo(opt):
    ...
    # 训练集加载器（适配分布式参数）
    train_loader, _dataset = create_dataloader(
        path=train_path,
        imgsz=imgsz,
    ...
    optimizer = smart_optimizer(
        model, opt.optimizer, lr0=opt.hyp["lr0"], momentum=opt.hyp["momentum"], decay=opt.hyp["weight_decay"]
    )

    def lf(x):
        return (1 - x / opt.epochs) * (1 - opt.hyp["lrf"]) + opt.hyp["lrf"]  # 线性调度
    ...
    for epoch in range(opt.epochs):
        ...
        for i, (imgs, targets, paths, _) in enumerate(pbar):
            ...
            with torch.cuda.amp.autocast(enabled=device.type != "cpu"):
                ...
                loss, _loss_items = compute_loss(preds, targets, feats)  # 传入特征计算能量损失
            ...
            if i % 10 == 0 and opt.topo_monitor and LOCAL_RANK in {-1, 0}:
                ...
                pbar.set_postfix(loss=f"{loss.item():.4f}", curvature=f"{curvature:.4f}", energy=f"{energy:.6f}")
    ...
    if LOCAL_RANK in {-1, 0}:
        final_ckpt = {"model": model.state_dict(), "opt": vars(opt), "hyp": opt.hyp, "epoch": opt.epochs}
